...
public class BucketSort {
    ...
    private static void insertionSort(List<Integer> bucket) {
        for (int i = 1; i < bucket.size(); i++) {
            ...
            SortingListener.notifyAccess(i, key);
            int j = i - 1;
            while (j >= 0) {
                int vj = bucket.get(j); 
                ...
                if (vj <= key)
                    break;
                int vj1 = bucket.get(j + 1); 
                SortingListener.notifyAccess(j + 1, vj1);
                SortingListener.notifySwap(j + 1, j, vj1, vj);
                bucket.set(j + 1, vj); 
            ...
            int vj1 = bucket.get(j + 1); 
            SortingListener.notifyAccess(j + 1, vj1);
            SortingListener.notifySwap(j + 1, j + 1, vj1, key);
            bucket.set(j + 1, key); 
            SortingListener.notifyAccess(j + 1, key);
        }
    }
    ...
    public static void sort(List<Integer> list) {
        ...
        int min = list.get(0); 
        SortingListener.notifyAccess(0, min);
        int max = list.get(0); 
        SortingListener.notifyAccess(0, max);
        for (int i = 1; i < list.size(); i++) {
            int v = list.get(i); 
            SortingListener.notifyAccess(i, v);
            if (v < min) min = v;
            if (v > max) max = v;
        }
        if (min == max) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (bi == list.size()) bi = list.size() - 1;
        ...
        for (int i = 0; i < list.size(); i++) {
            for (int v : buckets.get(i)) {
                ...
                SortingListener.notifyAccess(index, old);
                SortingListener.notifySwap(index, index, old, v);
                list.set(index, v); 
                SortingListener.notifyAccess(index, v);
                index++;
            }
        }
    }
